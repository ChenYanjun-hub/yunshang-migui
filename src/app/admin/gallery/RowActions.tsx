...
import { deleteGalleryPhoto, setGalleryPhotoStatus } from './actions';
...
export default function RowActions({
  ...
  function onDelete() {
    ...
    start(async () => {
      try { await deleteGalleryPhoto(id); } catch (e) { alert((e as Error).message); }
    });
  }

  function onStatus(next: 'draft' | 'published' | 'archived') {
    start(async () => {
      try { await setGalleryPhotoStatus(id, next); } catch (e) { alert((e as Error).message); }
    });
  }
  ...
  return (
    <div className="flex items-center gap-3" style={enFont}>
      <Link href={`/admin/gallery/${id}/edit`} className={`${btn} text-text-secondary hover:text-accent`}>
        Edit
      </Link>
      {status !== 'published' && (
